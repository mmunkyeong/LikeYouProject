...
public class LikeablePersonControllerTests {
    ...
    void t005() throws Exception {
        ...
        resultActions
                ...
                .andExpect(status().is2xxSuccessful())
                .andExpect(content().string(containsString("""
                        data-test="toInstaMember_username=insta_user4"</span>
                        """.stripIndent().trim())))
                .andExpect(content().string(containsString("""
                        data-test="toInstaMember_attractiveTypeDisplayName=외모"</span>
                        """.stripIndent().trim())))
                .andExpect(content().string(containsString("""
                        data-test="toInstaMember_username=insta_user100"</span>
                        """.stripIndent().trim())))
                .andExpect(content().string(containsString("""
                        data-test="toInstaMember_attractiveTypeDisplayName=성격"</span>
                        """.stripIndent().trim())));
        ;
    }
    ...
    @Test
    @DisplayName("호감취소(없는 거 취소, 취소가 안되어야함)")
    @WithUserDetails("user3")
